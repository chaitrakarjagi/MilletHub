...
import WholeMilletCard from "../components/WholeMilletCard";

export default function WholeMillet(){
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await API.get("/products/whole-millet");
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-4">
              Whole <span className="text-yellow-300">Millet</span>
            </h1>
            <p className="text-xl md:text-2xl mb-8 text-blue-100">
              Discover amazing whole grain products at unbeatable prices
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map(p => <WholeMilletCard key={p._id} product={p} />)}
          </div>
        )}
      </div>
    </div>
